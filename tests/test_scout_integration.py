...
def test_event_render_type_is_auto_assigned(qtbot):
    """
    Verify that RenderType.EVENT is automatically assigned for variables with var_type 'Event'.
    Scenario:
    1) start scout.py
    2) load test_inputs/vcd_extensions.vcd
    3) select 'main' scope in design tree (unified mode)
    4) Select 'EVENT_IN' variable, double click to add it to the waveform canvas
    5) Save session into yaml
    6) Verify that EVENT_IN has render_type 'event' in yaml file
    """
    from scout import WaveScoutMainWindow

    repo_root = Path(__file__).resolve().parent.parent
    vcd_path = repo_root / "test_inputs" / "vcd_extensions.vcd"
    assert vcd_path.exists(), f"VCD not found: {vcd_path}"
    ...
    # Root and main scope
    root = QModelIndex()
    main_idx = find_child_by_name(root, "main")
    assert main_idx and main_idx.isValid(), "'main' scope not found in design tree"

    # Expand main and find EVENT_IN
    design_view.expand(main_idx)
    qtbot.wait(50)
    event_idx = find_child_by_name(main_idx, "EVENT_IN")
    assert event_idx and event_idx.isValid(), "'EVENT_IN' variable not found under 'main' scope"

    # Add EVENT_IN to the session using the same code path as double-click
    node = event_idx.internalPointer()
    signal_node = window.design_tree_view._create_signal_node(node)
    assert signal_node is not None, "Failed to create SignalNode for EVENT_IN"
    window.design_tree_view.signals_selected.emit([signal_node])
    qtbot.wait(100)

    # Save session into YAML and verify render_type
    session = window.wave_widget.session
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        nodes = data.get("root_nodes", [])
        # Find EVENT_IN node
        ev_yaml = next((n for n in nodes if n.get("name", "").endswith("main.EVENT_IN")), None)
        assert ev_yaml is not None, "EVENT_IN node not found in saved YAML"
        fmt = ev_yaml.get("format") or {}
        assert fmt.get("render_type") == "event", f"Expected render_type 'event', got {fmt.get('render_type')}"
